...
```bash
CONFIG="${INFRA_DIR_LOCAL:-$(pwd)}/sysadmin-config.json"

if [ -f "$CONFIG" ]; then
    LANG_FROM_CONFIG=$(jq -r '.language // "ru"' "$CONFIG")
    TZ_FROM_CONFIG=$(jq -r '.operator.timezone // "UTC"' "$CONFIG")
else
    LANG_FROM_CONFIG="ru"
    TZ_FROM_CONFIG="UTC"
fi
...
TIMEZONE="${TIMEZONE:-$TZ_FROM_CONFIG}"
