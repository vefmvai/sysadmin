...
- **«`ufw limit` на SSH — бесплатная защита»** — нет, она дублирует fail2ban и мешает работе.
  ...
  частоту коннектов. Два слоя одной защиты не нужны. Диагностика на месте: симптом
  `Connection closed by remote host` на рукопожатии = UFW-лимит или fail2ban; `ufw status | grep 22`
  покажет `LIMIT` vs `ALLOW`, `fail2ban-client status sshd` — реальные баны.
