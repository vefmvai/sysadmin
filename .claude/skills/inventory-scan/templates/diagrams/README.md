...
Эти 4 шаблона лежат в публичном репо `sysadmin/` и копируются в `infra/inventory/diagrams/` оператора при первом запуске `/inventory-scan`. Дальше скилл заполняет их реальными данными из inventory.
...
3. **Стилизация через `classDef`** — цвет несёт смысл (зелёный = app, жёлтый = БД, синий = monitoring, розовый = VPN, серый = infrastructure).
