...
description: |
  ...
  outbound через подписку платного провайдера (сервера ИЗВЛЕКАЮТСЯ и СОХРАНЯЮТСЯ
  в infra, размечаются по странам, оператор выбирает страну выхода и пресет —
  «одна страна + авто-failover» или «один фиксированный сервер») ИЛИ через свой
...
| `PROVIDER_SLUG` | нет | `subscription` | Короткое имя провайдера для файла в infra (`blanc`, `nurvpn`, `panterra`) |
| `HWID` | нет | — | Hardware ID для HWID-locked подписок (Panterra/NurVPN/Happ). Добывается `resolve-hwid.sh`. Включает Happ-профиль заголовков |
| `HWID_MODE` | нет | `auto` | `auto` / `happ` (из plist установленного Happ) / `generate` (свой стабильный HWID). См. ADR-0010 |
...
**5A.1 — Извлечь сервера и СОХРАНИТЬ в infra (обязательно).**
Сервера из подписки — ценный ресурс оператора, они должны пережить сессию
(требование оператора). `parse-subscription.sh` парсит, `save-subscription-servers.sh`
сохраняет в `$INFRA/inventory/shared/vpn-subscriptions/<provider>.json` и размечает
по странам.
...
> 🔒 **HWID-замок (exit 3) — НЕ тупик, а развилка.** Если `parse-subscription.sh`
> вернул код **3**, подписка **живая**, но провайдер (Panterra, NurVPN, Happ-подобные)
> отдаёт реальный список серверов ТОЛЬКО зарегистрированному устройству по
> заголовку `X-Hwid` — нам пришла заглушка (`0.0.0.0` / «App not supported» /
> нулевые UUID). **НЕ сдавайся молча** (это та ошибка, что наблюдалась у ученика:
> агент сказал «нет способа обойти» и остановился). Вместо этого — сеньор-обёртка
> (раздел 4.3 персоны):
>
> 1. **Объясни замок простым языком:** «Подписка работает (виден расход трафика,
>    срок). Но провайдер привязал список серверов к "отпечатку устройства" (HWID) —
>    отдаёт его только зарегистрированному устройству через своё приложение. Нам
>    со стороны сервера прилетает заглушка.»
> 2. **Спроси про свободный слот** (ключевой вопрос — именно его не хватало):
>    «Сколько у тебя по тарифу устройств и сколько занято? Чтобы я мог
>    зарегистрировать "виртуальное устройство-сервер", нужен **один свободный слот**.
>    Если все заняты — освободи один (удали старое устройство в личном кабинете
>    провайдера).»
> 3. **Добудь HWID** (оба пути, `resolve-hwid.sh` сам выберет — ADR-0010):
>    ```bash
>    HWID="$(MODE=auto PROVIDER_SLUG="${PROVIDER_SLUG:-subscription}" \
>            INFRA_DIR="$INFRA_DIR" ./scripts/resolve-hwid.sh)"
>    # Путь A: если у оператора установлен Happ и подписка там работает —
>    #         берём HWID из его plist, СЛОТ НЕ ТРАТИМ.
>    # Путь B: иначе генерируем стабильный HWID (займёт 1 слот при первом запросе,
>    #         перезапуски его переиспользуют — новые слоты НЕ жжёт).
>    ```
>    Если оператор подтвердил, что Happ стоит и подписка там жива → `MODE=happ`.
>    Если хочет занять отдельный слот под агента → `MODE=generate`.
> 4. **Повтори запрос с HWID:**
>    ```bash
>    SUBSCRIPTION_URL=... HWID="$HWID" ./scripts/parse-subscription.sh > /tmp/subs.json
>    ```
>    Если снова код 3 — слота нет (все заняты) ИЛИ HWID не тот. Честно скажи
>    оператору: «Провайдер всё ещё отдаёт заглушку — скорее всего, нет свободного
>    слота. Освободи один и повтори.» **Не выдумывай сервера** (правило №1).

```bash
# Сохраняем в infra + размечаем страну каждого сервера (флаг/текст из тега).
# PROVIDER_SLUG — короткое имя провайдера (спросить у оператора или из домена подписки).
PARSED_JSON=/tmp/subs.json \
PROVIDER_SLUG="${PROVIDER_SLUG:-subscription}" \
INFRA_DIR="$INFRA_DIR" \
./scripts/save-subscription-servers.sh > /tmp/subs-enriched.json
# /tmp/subs-enriched.json — те же сервера + поле "country" (US/NL/DE/.../?)
```
...
- **Заведение ВСЕХ серверов подписки в один балансир скопом** — даёт «скачущий IP
  по странам» = бан аккаунта (платформы РФ-2026 детектят прыжки). Правильно:
  сохранить все в infra, но в outbound завести ТОЛЬКО выбранную оператором страну
  (Шаг 5A.2–5A.3).
...
- **Подписка HWID-locked — отдаёт заглушку вместо серверов** (Panterra, NurVPN,
  Happ-подобные; `parse-subscription.sh` exit 3) → НЕ тупик. Провайдер привязал
  список к HWID устройства. Алгоритм: (1) спросить оператора про **свободный слот**
  устройства в личном кабинете (нет свободного → освободить один); (2) добыть HWID
  через `resolve-hwid.sh` (путь A — из установленного Happ, слот не тратим; путь B —
  стабильный сгенерированный, займёт 1 слот); (3) повторить запрос с `HWID=...`.
  Подробный сценарий — Шаг 5A.1, блок «🔒 HWID-замок». См. ADR-0010. Если после
  HWID снова заглушка — нет свободного слота либо HWID не тот; честно сказать, не
  выдумывать сервера (правило №1).
...
- **Страна сервера = `?`** (тег без флага/текста) → `save-subscription-servers.sh`
  НЕ выдумывает страну. Определить по гео-IP хоста (`curl -s ipinfo.io/<host>/country`)
  или честно спросить оператора. Не подставлять «правдоподобную» страну (правило №1).
...
- `references/subscription-formats.md` — форматы подписок провайдеров (вкл. «Формат 5: HWID-locked»).
- `scripts/save-subscription-servers.sh` — сохранение серверов в infra + разметка по странам.
- `scripts/resolve-hwid.sh` — добыча HWID для HWID-locked подписок (путь A из Happ / путь B генерация).
- `decisions/0010-hwid-locked-subscriptions.md` — решение по HWID-замку и слотам.
- `../../knowledge/networking/_reference/subscription-mirroring.md` — зеркалирование подписки + детали HWID.
