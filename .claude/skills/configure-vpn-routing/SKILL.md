...
создаёт inbound. Протокол определён **автоматически из `vpn.server_role`** (Шаг 0a):
- `ru-server` → **VLESS-TCP** (вход внутри РФ, TSPU не пересекается — Reality НЕ нужен и запрещён).
...
`ru-server` + `vless-tcp` (безопаснее: лишний Reality хуже его отсутствия внутри РФ).

Запуск скрипта **всегда** с `SERVER_ROLE` — это включает guard:
...
> 🔒 **Guard в коде:** `create-vless-inbound.sh` блокирует `vless-reality` при
> `SERVER_ROLE=ru-server` (exit 2). Reality на РФ-входе — техническая ошибка
> (рефлекс персоны 3.8.5, `vpn-consultation-flow.md` §4). Текстовое правило
> подкреплено исполняемым барьером: даже если протокол подставлен ошибочно,
> скрипт не создаст Reality-inbound на РФ-сервере.
...
- **VLESS+Reality на РФ-сервере для входа клиентов** — техническая ошибка.
  Маршрут «клиент в РФ → сервер в РФ» не пересекает TSPU, маскировка не нужна
  (заработал бы и WireGuard). Для `ru-server` inbound — ВСЕГДА `vless-tcp`.
  Reality только на `foreign-server` или как outbound к загр.VPS. Подкреплено
  guard'ом в `create-vless-inbound.sh` (блок при `SERVER_ROLE=ru-server`).
  Наблюдённый инцидент (2026-05): агент в свободном режиме создал Reality-inbound
  на РФ-сервере, сбив оператора. См. рефлекс персоны 3.8.5.
