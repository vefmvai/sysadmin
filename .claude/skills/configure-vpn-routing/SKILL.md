...
Без него я не знаю, к какой панели подключаться (panel_url, web_base_path)
и где брать креды.
...
# 2) Секция vpn.* + panel_url + panel_web_base_path обязательны
...
(он заполняет эти поля по факту установки).

Сначала установи панель: /setup-vpn-panel SSH_TARGET=... DOMAIN=...
Затем возвращайся к /configure-vpn-routing.
EOF
    exit 1
fi

# 3) Параметры из CLI override'ят параметры из конфига
