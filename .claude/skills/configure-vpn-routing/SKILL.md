...
description: |
  ...
  routing rules по модели «золотая середина» (7 правил: private→direct,
  реклама→block, bittorrent→block, geoip:ru→direct, category-ru+regex→direct,
  остальное→upstream), массовое добавление клиентов с UUID. Использует REST
  API панели как основной путь.
...
`probeUrl=http://www.google.com/gen_204` и `probeInterval=30s`
(НЕ устаревший `pingConfig`).
