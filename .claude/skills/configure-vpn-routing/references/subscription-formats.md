...
## Формат 5: HWID-locked (привязка к устройству)

Современные платные провайдеры (**Panterra/Panterra VPN, NurVPN** и многие
Happ-обёртки) защищают список серверов **привязкой к HWID** (hardware ID —
«отпечаток устройства»). Реальный список отдаётся ТОЛЬКО зарегистрированному
устройству, которое шлёт правильный `X-Hwid` + набор заголовков Happ. Чужому/
незнакомому устройству приходит **заглушка**:

```
# вместо серверов — одно из:
0.0.0.0                                    # адрес-заглушка
"App not supported" / «Приложение не поддерживается»
vless://00000000-0000-0000-0000-000000000000@...   # нулевые UUID
```

Подписка при этом **живая** — в личном кабинете виден расход трафика и срок.
Просто список спрятан за HWID-замком + лимитом устройств (N слотов по тарифу).

**Как обойти (легально — у оператора есть доступ к подписке):**

1. **Свободный слот обязателен.** Чтобы зарегистрировать «устройство-сервер»,
   нужен свободный слот. Все заняты → освободить один в кабинете провайдера.
2. **Правильный HWID** — два пути (`scripts/resolve-hwid.sh`, ADR-0010):
   - **Путь A** — взять HWID уже установленного Happ оператора (из plist).
     Слот НЕ тратим — переиспользуем существующую регистрацию.
   - **Путь B** — сгенерировать стабильный HWID, сохранить в infra. Займёт 1 слот
     при первом запросе; перезапуски переиспользуют тот же HWID (новые слоты не жжёт).
3. **Happ-профиль заголовков** (`parse-subscription.sh` шлёт автоматически при
   заданном `HWID`):
   ```bash
   curl -sL --max-time 30 \
     -A 'Happ/4.9.0' \
...
`parse-subscription.sh` распознаёт заглушку и выходит с кодом **3** (а не глухим
«не найдено ссылок»), выдавая инструкцию про слот и HWID. Скилл на Шаге 5A.1
разворачивает это в сеньор-диалог. Подробности зеркалирования (раздача без
лимита) — `knowledge/networking/_reference/subscription-mirroring.md`.

> **Гео-блокировка поверх HWID.** Некоторые (NurVPN) дополнительно отдают
> 502/заглушку на не-РФ IP. Если HWID правильный, а ответ всё равно пустой —
...
| **Panterra VPN** | **HWID-locked** (Формат 5) | Без `X-Hwid` зарегистрированного устройства → заглушка `0.0.0.0`/«App not supported». Нужен свободный слот + HWID |
| **NurVPN** | **HWID-locked** + гео-блок | Реальный список только при `X-Hwid` (+ запрос с РФ-IP). `sid` дрожит между запросами |
| Happ-обёртка от других | HWID-locked (Формат 5) | Реальный список только зарегистрированному устройству через `X-Hwid` |
...
3. **Заглушка `0.0.0.0` / «App not supported» / нулевые UUID?** → HWID-locked
   (Формат 5). Добыть HWID (`resolve-hwid.sh`), убедиться в свободном слоте,
   повторить с `HWID=...`.
4. Попробовать разные User-Agent.
5. Спросить у провайдера, какой формат для xray/v2ray.
...
- `subscription-mirroring.md` (knowledge/networking/_reference/) — детали HWID-извлечения + зеркалирование подписки без лимита.
- `decisions/0010-hwid-locked-subscriptions.md` — решение по HWID-замку и логике слотов.
