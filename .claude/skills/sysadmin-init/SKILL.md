...
name: sysadmin-init
description: |
  Интерактивная первичная настройка и перенастройка агента-сисадмина под проект оператора.
  Создаёт sysadmin-config.json в корне репо: язык общения, менеджер паролей, серверы,
  включён ли мониторинг-стек, настройки бэкапов, Telegram-уведомления. Использует JSON Schema
  для валидации. Поддерживает первичный setup (нет конфига → интервью), идемпотентный no-op
  (конфиг есть → подсказка), и --reconfigure (показывает текущие значения, спрашивает что менять).
  Используй когда оператор говорит "настрой агента", "первый запуск", "у меня новый проект",
  "init agent", "/sysadmin-init", "хочу как у Василия", "перенастрой конфиг", "поменять язык
  агента", "переключить менеджер паролей".
when_to_use: |
  Первый запуск в новом проекте — обязательно перед install-monitoring-stack, setup-backups,
  audit-security (без конфига они выходят с STOP). После любых смен инфраструктуры (новый
  сервер, смена менеджера паролей, перенос на другой Telegram-бот) — через --reconfigure.
