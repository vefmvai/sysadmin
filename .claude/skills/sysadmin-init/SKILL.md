...
- Если родитель пути существует, но самой папки нет — это нормально, скилл не блокирует, оставляет WARN. Сценарий: оператор пишет `../infra`, а папки `infra/` пока нет рядом с `sysadmin/`. Это ожидаемая ситуация — агент создаст её сам при первой работе или оператор создаст вручную.
