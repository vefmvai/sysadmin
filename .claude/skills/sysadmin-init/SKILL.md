...
# 3. Чистим временный каталог — только теперь, когда всё подтверждено
rm -rf "$WORKDIR"
echo "Готово. Конфиг записан и проверен: $CONFIG_PATH"
```
...
6. **«Windows-ученица прошла настройку, осталась с мёртвым `infra.md` в папке курса
   вместо папки `infra/` с `sysadmin-config.json`» (инцидент 2026-05-24).** Причина:
