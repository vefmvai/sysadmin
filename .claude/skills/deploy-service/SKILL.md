...
- **`MODE=new`** → Шаги 3a (создание структуры из templates) → 4 (брифинг) → 5 (push-to-pull)
  → 5.5 (приёмка артефакта до переключения) → 6 (smoke) → 7 (nginx + TLS, если `DOMAIN`)
  → 8 (inventory)

- **`MODE=update`** → Шаг 3b (diff текущего состояния и нового) → 4 (брифинг с упором на что меняется)
  → 5 (push-to-pull) → 5.5 (приёмка артефакта) → 6 (smoke) → 8 (inventory)

- **`MODE=rollback`** → Шаг 3c (выбрать prev git ref) → 4 (брифинг отката) → 5 (rollback.sh с этим ref)
  → 6 (smoke). Шаг 5.5 пропускается: откатываемся на состояние, которое уже стояло на бою
...
## Шаг 5.5: Приёмка артефакта ДО переключения (блокер по ресурсам)

Шаг 1 проверяет **окружение** — свежесть inventory, бэкап, чистое дерево, домен. Содержимое
того, что реально поедет на сервер, до сих пор не проверял никто. А там живёт целый класс
дефектов, невидимых ни локально, ни в репозитории: **файл есть в рабочей копии, выглядит
целым, но в выкатку не едет** — помечен `export-ignore`, отфильтрован сборщиком, не попал в
образ. Ссылка на него при этом остаётся, и посетитель видит пустые рамки.

Ожог (отчёт пользователя агента, 2026-08-05): выкатка статического сайта перевела картинки в
WebP и убрала исходники из архива через `export-ignore`. Девять блоков на главной ссылались
на убранные файлы. Локально всё открывалось, репозиторий был чист, приёмка — зелёная.

**Принцип: судить о том, что будет на бою, по артефакту, а не по репозиторию.**

Поэтому проверка встраивается **в сам скрипт выкатки** — между «собрали/распаковали» и
«переключили каталоги», а не выполняется мной постфактум: при push-to-pull подмена
происходит внутри серверного скрипта, после неё проверять поздно.

| Что выкатываем | Что сверяем с артефактом |
|---|---|
| статика | ссылки на ресурсы: есть ли файл в **распакованном** каталоге сборки |
| образ | переменные окружения и точки монтирования, которых образ ждёт |
| пакет | зависимости, которых он требует |

**Две реакции, не одна.** Без разделения проверка бесполезна: живой сайт почти всегда
ссылается на ещё не сделанные разделы, блокирующая проверка рвала бы **каждую** выкатку — и
её снимут первой же рукой, а вместе с ней уйдёт защита от настоящего дефекта.

| Класс | Реакция | Почему |
|---|---|---|
| битый **ресурс** (картинка, стиль, скрипт, шрифт, документ) | 🔴 рвём выкатку до переключения | симптом сорвавшегося `export-ignore`; посетитель увидит пустые рамки |
| битая ссылка на **страницу/раздел** | строка в лог, не блокер | навигация на не сделанные разделы — обычное состояние живого сайта |

**Ключевая деталь реализации: пути ищутся строковыми литералами, а не разбором по тегам.**
Содержимое кавычек любого вида и `url(...)`, одинаково в `.html`, `.js` и `.css`. Разбор по
атрибутам разметки слеп к путям внутри скриптов на самой странице — на этом и погибли те
девять ссылок. Пути, собранные в коде из кусков (`${dir}/x.webp`), пропускаются **осознанно**:
статически они не проверяются, и блокировать по ним значило бы обещать больше, чем проверка
умеет.

**Саму проверку испытать на историческом дефекте** — коммит до починки, а не только на
здоровом каталоге и синтетике (якорь §3.11, `references/character-anchors.md`). Прогон на
здоровом состоянии доказывает, что код исполняется, а не что проверка видит дефект.

Не встроена в скрипт выкатки — задаю оператору три вопроса вслух и отвечаю на них фактами,
а не предположением: *что именно едет, чем это отличается от стоящего сейчас, и не ссылается
ли выкатываемое на то, чего в нём нет.*
