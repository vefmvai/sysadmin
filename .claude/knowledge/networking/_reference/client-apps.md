...
- **Импорт:** `happ://routing/add/{base64}` — добавить; `happ://routing/onadd/{base64}`
  ...
  стандартный base64 JSON-профиля (RFC 4648; URL-safe вариант в доке не указан —
  использовать стандартный, при проблеме проверить). Также QR-код и строка в подписке.
- ⚠️ Дока **не расписывает** точную логику `GlobalProxy` ↔ Direct/Proxy/Block-массивов
  (whitelist vs blacklist) — поведение проверять импортом на устройстве.
