...
>
> Гибкая маршрутизация **на устройстве** (raw route-правила в самом клиенте) —
> отдельный сценарий для энтузиастов: её честно умеет только sing-box-ядро
> (`routing-on-device-singbox.md`). **Hiddify для on-device split не годится** —
> он не исполняет произвольный raw route, а строит конфиг из подписки сам (см.
> §3.3). Это подтверждено практикой оператора (2026-05-22).
...
| Happ | 6504287215 | Xray-core (по умолчанию) + опция sing-box | трекает свежий Xray | **УДАЛЕНО** ~28.03.2026 |
...
| App Store (iOS/macOS) | id6504287215 — **удалён из RU App Store** ~28.03.2026 |
...
Поля routing-профиля (офиц. дока `happ.su/main/dev-docs/routing`):
- `DirectSites` / `ProxySites` / `BlockSites` + парные `DirectIp` / `ProxyIp` / `BlockIp`
- ссылки на geo-базы: `geosite:`, `geoip:` + произвольные CIDR; кастомные URL баз через
  `Geoipurl` / `Geositeurl`
- DNS: `RemoteDNS*`, `DomesticDNS*`, `DnsHosts`, `DomainStrategy`, `FakeDNS`
- `GlobalProxy` — мастер-переключатель режима

**Per-app routing на Android** (офиц. `app-management`): `per-app-proxy-mode` +
`per-app-proxy-list`, инверсия `per-app-proxy-list-invert` (всё кроме списка).

**Оговорка:** провайдер может **глобально запретить** пользовательский routing
параметром `routing-enable` (`Disable routing`) — на залоченной подписке ручных
правил может не быть. На своей инфре это не помеха.
